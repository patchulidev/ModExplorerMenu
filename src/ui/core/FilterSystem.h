...
namespace Modex
{
    enum class FilterBehavior {
        ...
        AUTOMATIC         // All children always show (no selection needed)
    };
        ...
        static std::shared_ptr<FilterNode> FromJson(const nlohmann::json& j) {
            ...
            if (j.contains("FilterProperty")) {
                ...
                if (filtersArray.is_array()) {
                    ...
                    node->behavior = FilterBehavior::SINGLE_SELECT; // TODO: Decide appropriate behavior for root
    ...
    public:
        // FilterSystem(const std::filesystem::path& a_path) : m_rootNode(nullptr), ConfigManager::m_file_path(a_path) 
        // {}
