...
namespace Modex
{
    void UIManager::OnShow()
	{
        ...
        m_menu = std::make_unique<ModexGUIMenu>();

        Menu* mainMenu = static_cast<Menu*>(m_windowStack.back().get());
        mainMenu->OpenWindow(this);
	...
	// FIXME: Is this a good location for this?
	UserData::Save();
    ...
    // TODO: need to decide wtf to do with tab button.
    ...
    {
        // Disable unecessary built-in navigation shortcuts
    }
    ...
    auto UIManager::MainWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) -> LRESULT
    {
        ...
        {
            case WM_DESTROY: {
                PrettyLog::Info("Window Destroyed, shutting down UIManager...");
    ...
    bool UIManager::DoInit(const RE::BSGraphics::RendererData &renderData, HWND hWnd)
    {
        ...
        io.IniFilename = NULL;
	ImGui::GetStyle().HoverDelayShort = 0.50f;   // Set short delay to 0.5 seconds
	ImGui::GetStyle().HoverDelayNormal = 0.75f;  // Set normal delay to 1.0 seconds
    ...
    // TODO: Probably not necessary anymore. Try handling via shortcuts in Popup component class.
    bool UIManager::InputHandler(ImGuiKey a_key)
    {
        ...
        // TODO: Probably need to specifically handle a whitelist for modifier keys for users wanting to set modifiers
