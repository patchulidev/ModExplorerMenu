...
namespace Modex
{
	...
	public:
		...
		static std::pair<T, bool> GetColor(std::string& a_str)
		{
			...
			PrettyLog::Error("Failed to parse color from .ini file! Expected format: {{R,G,B,A}} or #RRGGBBAA | Input: {}", a_str);
			return { T(), false };
		}
		...
		static std::pair<ImVec2, bool> GetVec2(std::string& a_str)
		{
			...
			PrettyLog::Error("Failed to parse ImVec2 from .ini file! Expected format: {x,y} | Input: {}", a_str);
			return { ImVec2(), false };
		}
